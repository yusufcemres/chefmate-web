...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (password.length < 8) { setError('Sifre en az 8 karakter olmali'); return; }
    setLoading(true);
    try {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 pt-16">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="font-heading text-2xl font-extrabold">Hesap Olustur</h1>
          <p className="text-sm text-text-muted mt-1">ChefMate ile mutfagini yonet</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5">Isim</label>
            <input
              type="text"
              value={displayName}
